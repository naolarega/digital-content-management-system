			...
			<div class="row">
				<div class="col-lg-8">
					<?php
						...
						if($music->price > 0){
							if(get_cookie('dcms_type') == 1){
								...
								if($payed_content){
									if(array_search($music->content_id, $payed_content) !== false){
										$is_payed = true;
									}
								}
							}
							echo '<h4 class="content-view-title">'.$music->content_name.' <span class="label label-info">payed content</span></h4>';
						}
						else{
							echo '<h4 class="content-view-title">'.$music->content_name.'</h4>';
						}
					?>
					<img width="218" height="218" class="music-thumbnail" src="<?php echo '/cdn/images/content-thumbnail/'.$music->thumbnail; ?>" />
					<audio class="music-player" autoplay controls>
					<?php
						if($music->price > 0 and get_cookie('dcms_type') == 1){
							if($is_payed){
								echo '<source id="music-source" src="/cdn/user-content/musics/'.$music->file_name.'" /></source>';
							}
						}
						if($music->price > 0 and get_cookie('dcms_type') == 2){
							if($music->user_id == $user->user_id){
								echo '<source id="music-source" src="/cdn/user-content/musics/'.$music->file_name.'" /></source>'; 
							}
						}
						else{
					...
					if(get_cookie('dcms_username') != null){
						...
						echo '<input id="creator_id" type="hidden" value="'.$music->user_id.'" />';
						echo '<br/><a class="btn btn-default" data-toggle="modal" data-target="#comment-modal">
						...
						if($music->price > 0 and get_cookie('dcms_type') == 1 and !$is_payed){
							echo '<input type="hidden" class="payed-content" value="'.$music->content_id.'" />';
